...
class ExecSim:
    def __init__(self, net_name, src_dir, tp_dir, tp, traf, ijr, nvc, use_trace_file=False, trace_file=""):
    ...
    def execsim(self):
        ...
        bprocs = [BsimOpen(net_name, src_dir, topo_dir, topo, traffic, ijr, num_vcs, use_trace_file, trace_file, "deadlock", 300) for ijr in ij_rates]
...
class BsimOpen:
    ...
    def bsim_open(self):
        ...
        outf.write(CFG_TEMPLATE % (net_name, "%s.tp" % topo_path, "%s.rt" % topo_path, traffic, ij_rate, num_vcs, utrace_int, trace))
        outf.close()
