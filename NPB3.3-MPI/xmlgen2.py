...
route_node_router = """  <route src='n{1}' dst='s{0}'>
   <link_ctn id='linkn{1}s{0}'/>
  </route>
"""

route_router_node = """  <route src='s{0}' dst='n{1}'>
   <link_ctn id='linkn{1}s{0}'/>
  </route>
"""
...
if __name__ == "__main__":
    import sys
    edge, npr = sys.argv[1], int(sys.argv[2])
    print(header)
    ...
    with open(out_xml, "w") as f:
        ...
        for n in nodelist:
            r = math.ceil(n / npr)
            f.write(route_node_router.format(r, n))

        for r1, r2 in edgelist:
            f.write(route_router_router.format(r1, r2))
