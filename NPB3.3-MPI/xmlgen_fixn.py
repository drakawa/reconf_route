...
if __name__ == "__main__":
    import sys
    edge, npr, maxnode = sys.argv[1], int(sys.argv[2]), int(sys.argv[3])
    print(header)
    ...
    n_nodes = min(n_routers * npr, maxnode)
    ...
    while idx < len(nodelist):
        tmp_nodes = nodelist[idx:(idx+len(routerlist))]
        for r_idx, n in enumerate(tmp_nodes):
            router_n[n] = routerlist[r_idx]
        idx += len(routerlist)
