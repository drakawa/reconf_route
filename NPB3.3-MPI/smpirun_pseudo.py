...
for a, s, n in npb_params:
    for b in bases:
        if a not in a_exclude:
            ...
            print("timeout -s sigint 14400 smpirun --cfg=smpi/privatize_global_variables:yes -platform {0}.xml -hostfile {0}.txt bin/{1}.{2}.{3} > {0}_{1}_{2}_{3}.log".format(b, a, s, n))
        # exit(1)
