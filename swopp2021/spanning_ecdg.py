...
if __name__ == "__main__":
    ...
    for n, d, s in it.product(ns, ds, ss):
        ...
        cdg_src = 0
        bfs_edges = nx.bfs_edges(G, source=cdg_src)
        print(list(bfs_edges))

        G_aspl = nx.average_shortest_path_length(G)
        G_diam = nx.diameter(G)
        print("Diam:", G_diam, "ASPL:", G_aspl)

        H = GenSpanningCDG(G, cdg_src).gen_spanning_cdg()
        H_sorted_nodes = sorted(H.nodes, key=lambda x: (x[1],x[0]))
        print(H)
        print(H_sorted_nodes)

        H_sc = nx.to_scipy_sparse_matrix(H, nodelist=H_sorted_nodes)
        H_scsp = shortest_path(H_sc)

        sd_offset = 1
        H_scsp_sd = H_scsp[0:n,n:2*n] - sd_offset
        print(H_scsp_sd)
        print(np.max(H_scsp_sd), np.average(H_scsp_sd))

        H_full = GenChannelGraph(d, n, s).gen_channel_graph()
        H_full_sorted_nodes = sorted(H_full.nodes, key=lambda x: (x[1],x[0]))
        print(H_full)
        print(H_full_sorted_nodes)

        H_full_sc = nx.to_scipy_sparse_matrix(H_full, nodelist=H_full_sorted_nodes)
        H_full_scsp = shortest_path(H_full_sc)

        sd_offset = 1
        H_full_scsp_sd = H_full_scsp[0:n,n:2*n] - sd_offset
        print(H_full_scsp_sd)
        print(np.max(H_full_scsp_sd), np.average(H_full_scsp_sd))
        H_full_sumlen = np.sum(H_full_scsp_sd)
        print(H_full_sumlen / (n * (n-1)))
